...
def render_analyze():
    ...
    st.markdown('<p class="subheader">Загрузите договор для автоматического анализа на риски</p>', unsafe_allow_html=True)
    ...
    if uploaded_file:
        ...
        # Кнопка анализа
        if st.button("🔍 Начать анализ рисков", type="primary", use_container_width=True):
                    ...
                    for i, risk in enumerate(risks, 1):
                        ...
                        with st.container():
                            ...
                            <div class="risk-card {risk_class}">
                                ...
                                <small>Уровень: {risk.risk_level.value.upper()}</small>
                            </div>
                            """, unsafe_allow_html=True)
                            ...
                            with st.expander(f"Подробнее о риске {i}"):
                                ...
                                if risk.proposed_text:
                                    ...
                                    st.info(risk.proposed_text)
                    
                    # Кнопка скачивания отчёта
                    report = f"""
# ОТЧЁТ ПО АНАЛИЗУ ДОГОВОРА
...
"""
                    ...
                    st.download_button(
                        "📥 Скачать отчёт",
                        data=report,
                        file_name=f"risk_analysis_{datetime.now().strftime('%Y%m%d')}.txt",
                        mime="text/plain"
                    )
